    ...
    SubShader
    {
        Pass
        {
            ...
            VsOut vert(VsIn i)
            {
                ...
                o.texcoord0 = (i.texcoord0 +  rand(i.texcoord0 * _rythm) * 0.00002) * _zoom;
                o.texcoord1 = i.texcoord0 - 0.5;
                return o;
            }
            ...
            float4 frag(VsOut i) : COLOR
            {
                ...
                float sphere_mask   = repetition(float3(uv.x + rand(i.texcoord0.xy + _SinTime.y) * _rythm * 200, 
														uv.y + rand(i.texcoord0.xy) * _rythm * 200, -0.5), 
												float3(_dist, _dist, 1.));
                //float dist_sqrt     = sqrt(dot(i.texcoord1.xy, i.texcoord1.xy)) * 2.0;
                // Hard edges 
                sphere_mask = sphere_mask >= 1;

                //float4 col = float4(sphere_mask, sphere_mask, sphere_mask, sphere_mask);
                //float4 col2 = float4(1., 1., 1., 1.);
                //col = lerp(WHITE, BLACK, sphere_mask);
                /*if(fmod(_rythm, 2) > 1)
                    col2 = lerp(_col0, _col1, sphere_mask + _rythm * rand(i.texcoord0.xy) * 100);
                else*/
                //col2 = lerp(_col2, _col3, sphere_mask);
                            
                //col = lerp(col, col2, _rythm * rand(i.texcoord0.xy) * 0.3);
                //col =  lerp(col, BLACK, dist_sqrt);
                
                return lerp(_col2, _col3, sphere_mask);
            }
            ENDCG
        }
    }
